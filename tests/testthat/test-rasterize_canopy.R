...
test_that("rasterize_canopy with p2r() returns a georeferenced RasterLayer", {
  ...
  expect_equivalent(dim(x), c(25,25))
  expect_equal(sf::st_bbox(x), expected_bbox)
...
test_that("rasterize_canopy with p2r() works with na.fill", {
  ...
  expect_equal(sum(is.na(x[[1]])), 118L)

  x   <- rasterize_canopy(las, 4, p2r(na.fill = knnidw(3)))

  expect_equal(sum(is.na(x[[1]])), 0L)
})
...
test_that("rasterize_canopy tin works both with LAS and LAScatalog", {
  ...
  b <- sf::st_buffer(sf::st_as_sfc(st_bbox(x)), -1)
  x <- x[b]
  y <- y[b]
  ...
  expect_equivalent(dim(x), c(88,88))
  expect_equivalent(st_bbox(x), expected_bbox)
  expect_equal(names(x), "Z")
  expect_equal(x[[1]], y[[1]], tolerance = 0.00065)
})
...
test_that("rasterize_canopy pit-free works both with LAS and LAScatalog", {
  ...
  expect_equal(names(x), "Z")
  expect_equal(x[[1]], y[[1]], tolerance = 0.00079)
})
...
test_that("triangulation does not return negative values", {
  ...
  expect_equal(min(chm[[1]], na.rm = T), 0)
})
