...
Obviously it is always possible to modify an R object by overpassing such simple restrictions. In this case the user will break something internally and a correct output is not guaranteed.
...
![](https://raw.githubusercontent.com/Jean-Romain/lidR/master/others/CATALOG/mapview-vignette.png).
...
Load a catalog. Process small chunks of 200 x 200 meters at a time. Each chunk is loadded with an extra 20 m buffer. Use 3 cores. Returns a Shapefile into R.
...
Load a catalog. Load a shapefile of plot centers. Extract the plots and immediately write them into a file named after the coordinates of the plot and an attributes of the sahpefile (here `PLOTID` if such an attribute exists in the shapefile). Returns a lightweight LAScatalog.
