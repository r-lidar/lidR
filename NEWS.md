...
2. When a VRT is writen on disk the default NA value (`vrtnodota` in `gdalUtils::gdalbuildvrt`) is set to `-.Machine$double.xmax` instead of `-Inf` that seems to be an invalid value on 32 bits arch.
