...
#' is used to retrieve each scanline. An attribute '\code{scanlineID}' is added in the \code{LAS} object}
...
laspulse = function(las)
{
  ...
  las@data[["pulseID"]] <- .lagisdiff(las@data[["gpstime"]])
  return(las)
}
...
lasflightline = function(las, dt = 30)
{
  ...
  gpstime <- NULL
  data.table::setorder(las@data, gpstime)
  las@data[["flightlineID"]] <- .lagissup(las@data[["gpstime"]], dt)
  return(las)
}
...
lasscanline = function(las)
{
  ...
  gpstime <- NULL
  data.table::setorder(las@data, gpstime)
  las@data[["scanlineID"]] <- .lagisdiff(las@data[["ScanDirectionFlag"]])
  return(las)
}
