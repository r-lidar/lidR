    ...
    if (header->evlrs)
    {
      for (int i = 0; i < (int)header->number_of_extended_variable_length_records; i++)
      {
        if (moveEVLRtoVLR)
        {
          ...
          {
#ifdef _WIN32
            Rcpp::Rcerr << "large EVLR with user ID '" << header->evlrs[i].user_id << "' and record ID " << header->evlrs[i].record_id << " with payload size " << header->evlrs[i].record_length_after_header << " not moved to VLRs." << std::endl;
#else
            Rcpp::Rcerr << "large EVLR with user ID '" << header->evlrs[i].user_id << "' and record ID " << header->evlrs[i].record_id << " with payload size " << header->evlrs[i].record_length_after_header << " not moved to VLRs." << std::endl;
#endif
          }
        ...
        else if (strcmp(header->evlrs[i].user_id, "LASF_Projection") == 0)
        {
          ...
          {
            Rcpp::Rcerr << "unknown LASF_Projection EVLR with record ID " << header->evlrs[i].record_id << " not moved to VLRs." << std::endl;
          }
        }
      }
    }
...
BOOL LASwriterCompatibleUp::open(LASheader* header, LASwriteOpener* laswriteopener)
{
  ...
  {
    Rcpp::Rcerr << "ERROR: no compatibility VLR in header" << std::endl; 
    return FALSE;
  }
  // the compatibility VLR must have the right length
  if (compatibility_vlr->record_length_after_header != (2+2+4+148))
  {
    Rcpp::Rcerr << "ERROR: compatibility VLR has " << compatibility_vlr->record_length_after_header << " instead of " << 2+2+4+148 << " bytes in payload" << std::endl;
    return FALSE;
  }
  I32 index_scan_angle = header->get_attribute_index("LAS 1.4 scan angle");
  if (index_scan_angle == -1)
  {
    Rcpp::Rcerr << "ERROR: attribute \"LAS 1.4 scan angle\" is not in EXTRA_BYTES" << std::endl;
    return FALSE;
  }
  start_scan_angle = header->get_attribute_start(index_scan_angle);
  ...
  {
    Rcpp::Rcerr << "ERROR: attribute \"LAS 1.4 extended returns\" is not in EXTRA_BYTES" << std::endl;
    return FALSE;
  }
  start_extended_returns = header->get_attribute_start(index_extended_returns);
  ...
  {
    Rcpp::Rcerr << "ERROR: attribute \"LAS 1.4 classification\" is not in EXTRA_BYTES" << std::endl;
    return FALSE;
  }
  start_classification = header->get_attribute_start(index_classification);
  ...
  if (index_scan_angle == -1)
  {
    Rcpp::Rcerr << "ERROR: attribute \"LAS 1.4 flags and channel\" is not in EXTRA_BYTES" << std::endl;
    return FALSE;
  }
  start_flags_and_channel = header->get_attribute_start(index_flags_and_channel); 
  ...
  {
    Rcpp::Rcerr << "ERROR: compatibility mode version " << compatible_version << " not implemented" << std::endl;
    return FALSE;
  }
  U32 unused;
  ...
  if (unused != 0)
  {
    Rcpp::Rcerr << "WARNING: unused is " << unused << " instead of 0" << std::endl;
  }
