...
test_that("lasmetrics works with LAS", {
  x = lasmetrics(las, mean(Z))
  expect_equal(x, 975.9, tol = 0.1)

  x = lasmetrics(las, ~mean(Z))
  expect_equal(x, 975.9, tol = 0.1)

  x = lasmetrics(las, ~list(mean(Z), max(Z)))
  expect_is(x, "list")
...
test_that("lasmetrics works with catalog_apply", {
  x = catalog_apply(ctg, lasmetrics, func = ~mean(Z))
  expect_is(x, "list")
  expect_equal(length(x), 2L)
  expect_equal(x[[1]], 975.9, tol = 0.1)
  expect_equal(x[[2]], 975.9, tol = 0.1)

  x = catalog_apply(ctg, lasmetrics, func = ~list(mean(Z), max(Z)))
  expect_is(x, "list")
  expect_equal(length(x), 2L)
  expect_is(x[[1]], "list")
  expect_is(x[[2]], "list")
