...
BOOL LASreaderDTM::open(const CHAR* file_name)
{
  ...
  {
    throw std::runtime_error(std::string("ERROR: reading 21 byte signature for '"));
    return FALSE;
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: cannot open DTM file '"));
    return FALSE;
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: reading 61 byte description for '"));
    return FALSE;
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: reading 4 byte version for '"));
    return FALSE;
  }
  ...
  if (version != 3.1f)
  {
    throw std::runtime_error(std::string("WARNING: expected 3.1 but version is "));
  }
  ...
  if (rotation != 0.0)
  {
    throw std::runtime_error(std::string("WARNING: expected 0.0 but rotation is "));
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte horizontal_units for '"));
    return FALSE;
  }

  if (fread(&vertical_units, 2, 1, file) != 1)
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte vertical_units for '"));
    return FALSE;
  }

  if (fread(&data_type, 2, 1, file) != 1)
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte data_type for '"));
    return FALSE;
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte coordinate_zone for '"));
    return FALSE;
  }

  if (fread(&horizontal_datum, 2, 1, file) != 1)
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte horizontal_datum for '"));
    return FALSE;
  }

  if (fread(&vertical_datum, 2, 1, file) != 1)
  {
    throw std::runtime_error(std::string("ERROR: reading 2 byte vertical_datum for '"));
    return FALSE;
  }
  ...
  if ((coordinate_system == 2) && (coordinate_zone >= 1) && (coordinate_zone <= 160)) // UTM
  {
    ...
    if (horizontal_datum == 1) // GEO_DATUM_NAD27
    {
      ...
      {
        throw std::runtime_error(std::string("UTM zone "));
      }
    }
  ...
  else if (coordinate_system == 3) // state plane
  {
    ...
    if (horizontal_datum == 2) // GEO_DATUM_NAD83
    {
      ...
      case GCTP_NAD83_Puerto_Rico:
        ...
        break;
      default:
        throw std::runtime_error(std::string("state plane NAD83 zone "));
      }
    }
  ...
  if (xdim <= 0)
  {
    throw std::runtime_error(std::string("WARNING: xdim was "));
    xdim = 1;
  }
  ...
  {
    ydim = 1;
    throw std::runtime_error(std::string("WARNING: ydim was "));
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR: unknown data type "));
    return FALSE;
  }
