# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

C_algo_dalponte <- function(Image, Seeds, th_seed, th_crown, th_tree, DIST) {
    .Call(`_lidR_C_algo_dalponte`, Image, Seeds, th_seed, th_crown, th_tree, DIST)
}

algo_li2012 <- function(las, dt1, dt2, th_tree, R, progressbar = FALSE) {
    .Call(`_lidR_algo_li2012`, las, dt1, dt2, th_tree, R, progressbar)
}

fast_table <- function(x, size = 5L) {
    .Call(`_lidR_fast_table`, x, size)
}

fast_countequal <- function(x, t) {
    .Call(`_lidR_fast_countequal`, x, t)
}

fast_countbelow <- function(x, t) {
    .Call(`_lidR_fast_countbelow`, x, t)
}

fast_countover <- function(x, t) {
    .Call(`_lidR_fast_countover`, x, t)
}

fast_extract <- function(r, x, y, xmin, ymin, res) {
    .Call(`_lidR_fast_extract`, r, x, y, xmin, ymin, res)
}

roundc <- function(x, digit = 0L) {
    .Call(`_lidR_roundc`, x, digit)
}

initialAngleValuesForProfileEvaluation <- function(generatedAngleValues, nbProfiles, init_alpha) {
    .Call(`_lidR_initialAngleValuesForProfileEvaluation`, generatedAngleValues, nbProfiles, init_alpha)
}

nextAngleValuesForProfileEvaluation <- function(generatedAngleValues, nbProfiles, init_alpha) {
    .Call(`_lidR_nextAngleValuesForProfileEvaluation`, generatedAngleValues, nbProfiles, init_alpha)
}

> <- function(disc, parameters, nbPoints) {
    .Call(`_lidR_>`, disc, parameters, nbPoints)
}

createCornerCoordinates <- function(center, radius, widthProfile, coordRef) {
    invisible(.Call(`_lidR_createCornerCoordinates`, center, radius, widthProfile, coordRef))
}

cart2pol_vec <- function(points, center, length, result) {
    invisible(.Call(`_lidR_cart2pol_vec`, points, center, length, result))
}

pol2cart_vec <- function(points, center, length, Xsign, result) {
    invisible(.Call(`_lidR_pol2cart_vec`, points, center, length, Xsign, result))
}

generateProfile_vec <- function(parameters, coordCylindRef, angleProfile, length, center, Xsign, subProfileRight, subProfileLeft, result, test2) {
    invisible(.Call(`_lidR_generateProfile_vec`, parameters, coordCylindRef, angleProfile, length, center, Xsign, subProfileRight, subProfileLeft, result, test2))
}

showMedian <- function(array, size) {
    .Call(`_lidR_showMedian`, array, size)
}

findGap_vec <- function(subProfile, length, sensitivity = 6) {
    .Call(`_lidR_findGap_vec`, subProfile, length, sensitivity)
}

find_tree_polygon_vec <- function(disc, nps, SENSITIVITY, MDCW, epsilon, CLc, CLs, Oc, Os, angleRefCone, angleRefSphere, center, radius) {
    .Call(`_lidR_find_tree_polygon_vec`, disc, nps, SENSITIVITY, MDCW, epsilon, CLc, CLs, Oc, Os, angleRefCone, angleRefSphere, center, radius)
}

Cpp_grid_canopy <- function(las, res, subcircle = 0) {
    .Call(`_lidR_Cpp_grid_canopy`, las, res, subcircle)
}

knn <- function(X, Y, x, y, k) {
    .Call(`_lidR_knn`, X, Y, x, y, k)
}

knnidw <- function(X, Y, Z, x, y, k, p) {
    .Call(`_lidR_knnidw`, X, Y, Z, x, y, k, p)
}

C_LocalMaximaMatrix <- function(image, ws, th) {
    .Call(`_lidR_C_LocalMaximaMatrix`, image, ws, th)
}

C_LocalMaximaPoints <- function(las, ws, min_height, displaybar = FALSE) {
    .Call(`_lidR_C_LocalMaximaPoints`, las, ws, min_height, displaybar)
}

MorphologicalOpening <- function(X, Y, Z, resolution, displaybar = FALSE) {
    .Call(`_lidR_MorphologicalOpening`, X, Y, Z, resolution, displaybar)
}

point_in_polygon <- function(vertx, verty, pointx, pointy) {
    .Call(`_lidR_point_in_polygon`, vertx, verty, pointx, pointy)
}

points_in_polygon <- function(vertx, verty, pointx, pointy) {
    .Call(`_lidR_points_in_polygon`, vertx, verty, pointx, pointy)
}

points_in_polygons <- function(vertx, verty, pointx, pointy, displaybar = FALSE) {
    .Call(`_lidR_points_in_polygons`, vertx, verty, pointx, pointy, displaybar)
}

tinfo <- function(M, X) {
    .Call(`_lidR_tinfo`, M, X)
}

tsearch <- function(x, y, elem, xi, yi, diplaybar = FALSE) {
    .Call(`_lidR_tsearch`, x, y, elem, xi, yi, diplaybar)
}

update_list_by_ref <- function(x, name, value) {
    invisible(.Call(`_lidR_update_list_by_ref`, x, name, value))
}

