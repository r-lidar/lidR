...
#' @param th numeric. Threshold below which a pixel or a point cannot be a local maxima.
#' Default 2.
#'
...
#'@export
tree_detection.LAS = function(x, ws = 3, th = 2)
{
  if (ws <= 0) stop("ws should be stricly positive", call. = FALSE)
  if (th <  0) stop("th should be positive", call. = FALSE)
...
#'@export
tree_detection.lasmetrics = function(x, ws, th = 2)
{
  x = as.raster(x)
  return(tree_detection(x, ws, th))
}
...
#'@export
tree_detection.matrix = function(x, ws, th = 2)
{
  if (ws < 3)
