...
#' create a single 64 bits number with a bitwise operation. First, XY coordinates are converted to
#' 32 bits integers using the scales and offsets of the point-cloud. For example, if the apex is at
#' (10.32, 25.64) with a scale factor of 0.01 and an offset of 0, the 32 bits integer coordinates are
#' X = 1032 and Y = 2564. Their binary representation are respectively (here displayed on 16 bits)
#' 0000010000001000 and 0000101000000100. X is shifted by 32 bits an becomes a 64 bits integer. Y is kept
#' as is and the binary representation are unionized into a 64 bits integer like (here displayed on 32 bit)
#' 00000100000010000000101000000100 that is guaranteed to be unique. However R
#' does not support 64 bits integers. The previous steps are done at C++ level and the 64 bits binary
...
#' interpreted as integer becomes 3.34164837074751323479078607289E-316 if interpreted as a decimal number.
...
#' meaningless numbers. But at least it works and deals with R limitations.
#' @md
