...
A simple (perhaps too simplistic) way to detect outliers is to measure the 95th percentile of height in 10 x 10-m pixels (area-based approach) and then remove the points that are above the 95th percentile in each pixel plus, for example, 20%. This can easily be built in lidR using `grid_metrics`, `merge_spatial` and `filter_points`, and should work either on a normalized or a raw point cloud. Let's create a function method `filter_noise` for LAS objects:
...
filter_noise.LAS = function(las, sensitivity)
{
  ...
  las <- filter_points(las, Z < p95*sensitivity)
  las$p95 <- NULL
  return(las)
}
```
...
filter_noise.LAScluster = function(las, sensitivity)
{
  ...
  las <- filter_points(las, buffer == 0)           # Don't forget to remove the buffer
