...
BOOL LASwriterLAS::open(ByteStreamOut* stream, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  ...
  if (laszip)
  {
    ...
    {
      throw std::runtime_error(std::string("ERROR: point type "));
      return FALSE;
    }
  }
  else
  ...
  {
    throw std::runtime_error(std::string("WARNING: header->version_major is "));
    version_major = 1;
  }
  ...
  if (version_minor > 4)
  {
    throw std::runtime_error(std::string("WARNING: header->version_minor is "));
    version_minor = 4;
  }
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing header->number_of_points_by_return["));
      return FALSE;
    }
  }
  ...
  if (version_minor >= 3)
  {
    ...
    {
#ifdef _WIN32
      throw std::runtime_error(std::string("WARNING: header->start_of_waveform_data_packet_record is "));
#else
      throw std::runtime_error(std::string("WARNING: header->start_of_waveform_data_packet_record is "));
#endif
  ...
  if (version_minor >= 4)
  {
    ...
    {
#ifdef _WIN32
      throw std::runtime_error(std::string("WARNING: EVLRs not supported. header->start_of_first_extended_variable_length_record is "));
#else
      throw std::runtime_error(std::string("WARNING: EVLRs not supported. header->start_of_first_extended_variable_length_record is "));
#endif
    ...
    {
      throw std::runtime_error(std::string("WARNING: EVLRs not supported. header->number_of_extended_variable_length_records is "));
    ...
    for (i = 0; i < 15; i++)
    {
      ...
      {
        throw std::runtime_error(std::string("ERROR: writing header->extended_number_of_points_by_return["));
        return FALSE;
      }
    }
  }
  ...
  if (header->user_data_in_header_size)
  {
    if (header->user_data_in_header)
    {
      ...
      {
        throw std::runtime_error(std::string("ERROR: writing "));
        return FALSE;
      }
    }
    else
    {
      throw std::runtime_error(std::string("ERROR: there should be "));
      return FALSE;
    }
  }
...
//      throw std::runtime_error(std::string("WARNING: wrong header->vlrs["));
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlrs["));
      return FALSE;
    }
  ...
  if (laszip)
  {
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing reserved "));
      return FALSE;
    }
    U8 user_id[16] = "laszip encoded\0";
    if (!stream->putBytes((U8*)user_id, 16))
    {
      throw std::runtime_error(std::string("ERROR: writing user_id "));
      return FALSE;
    }
    U16 record_id = 22204;
    if (!stream->put16bitsLE((U8*)&(record_id)))
    {
      throw std::runtime_error(std::string("ERROR: writing record_id "));
      return FALSE;
    }
    U16 record_length_after_header = laszip_vlr_data_size;
    if (!stream->put16bitsLE((U8*)&(record_length_after_header)))
    {
      throw std::runtime_error(std::string("ERROR: writing record_length_after_header "));
      return FALSE;
    }
    char description[32];
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing description "));
      return FALSE;
    }
    // write the data following the header of the variable length record
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing compressor "));
      return FALSE;
    }
    if (!stream->put16bitsLE((U8*)&(laszip->coder)))
    {
      throw std::runtime_error(std::string("ERROR: writing coder "));
      return FALSE;
    }
    if (!stream->putByte(laszip->version_major))
    {
      throw std::runtime_error(std::string("ERROR: writing version_major "));
      return FALSE;
    }
    if (!stream->putByte(laszip->version_minor))
    {
      throw std::runtime_error(std::string("ERROR: writing version_minor "));
      return FALSE;
    }
    if (!stream->put16bitsLE((U8*)&(laszip->version_revision)))
    {
      throw std::runtime_error(std::string("ERROR: writing version_revision "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(laszip->options)))
    {
      throw std::runtime_error(std::string("ERROR: writing options "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(laszip->chunk_size)))
    {
      throw std::runtime_error(std::string("ERROR: writing chunk_size "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(laszip->number_of_special_evlrs)))
    {
      throw std::runtime_error(std::string("ERROR: writing number_of_special_evlrs "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(laszip->offset_to_special_evlrs)))
    {
      throw std::runtime_error(std::string("ERROR: writing offset_to_special_evlrs "));
      return FALSE;
    }
    if (!stream->put16bitsLE((U8*)&(laszip->num_items)))
    {
      throw std::runtime_error(std::string("ERROR: writing num_items "));
      return FALSE;
    }
    for (i = 0; i < laszip->num_items; i++)
    {
      ...
      {
        throw std::runtime_error(std::string("ERROR: writing type "));
        return FALSE;
      }
      if (!stream->put16bitsLE((U8*)&(laszip->items[i].size)))
      {
        throw std::runtime_error(std::string("ERROR: writing size "));
        return FALSE;
      }
      if (!stream->put16bitsLE((U8*)&(laszip->items[i].version)))
      {
        throw std::runtime_error(std::string("ERROR: writing version "));
        return FALSE;
      }
    }
  ...
  if (header->vlr_lastiling)
  {
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->level "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->level_index)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->level_index "));
      return FALSE;
    }
    if (!stream->put32bitsLE(((U8*)header->vlr_lastiling)+8))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->implicit_levels "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->min_x)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->min_x "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->max_x)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->max_x "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->min_y)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->min_y "));
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->max_y)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lastiling->max_y "));
      return FALSE;
    }
  }
  ...
  if (header->vlr_lasoriginal)
  {
    ...
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->number_of_point_records "));
      return FALSE;
    }
    for (j = 0; j < 15; j++)
    {
      ...
      {
        throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->number_of_points_by_return["));
        return FALSE;
      }
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->min_x)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->min_x "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->max_x)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->max_x "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->min_y)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->min_y "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->max_y)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->max_y "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->min_z)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->min_z "));
      return FALSE;
    }
    if (!stream->put64bitsLE((U8*)&(header->vlr_lasoriginal->max_z)))
    {
      throw std::runtime_error(std::string("ERROR: writing header->vlr_lasoriginal->max_z "));
      return FALSE;
    }
  }
...
BOOL LASwriterLAS::update_header(const LASheader* header, BOOL use_inventory, BOOL update_extra_bytes)
{
  ...
  if (use_inventory)
  {
    ...
    else if (inventory.extended_number_of_point_records > U32_MAX)
    {
      ...
      {
        throw std::runtime_error(std::string("WARNING: too many points in LAS "));
        number = U32_MAX;
      }
    }
    else
    ...
    for (i = 0; i < 5; i++)
    {
      ...
      if (!stream->put32bitsLE((U8*)&number))
      {
        throw std::runtime_error(std::string("ERROR: updating inventory.number_of_points_by_return["));
        return FALSE;
      }
    }
    ...
    if (header->version_minor >= 4)
    {
      ...
      for (i = 0; i < 15; i++)
      {
        ...
        {
          throw std::runtime_error(std::string("ERROR: updating header->extended_number_of_points_by_return["));
          return FALSE;
        }
      }
    }
  ...
  else
  {
    ...
    for (i = 0; i < 5; i++)
    {
      ...
      if (!stream->put32bitsLE((U8*)&number))
      {
        throw std::runtime_error(std::string("ERROR: updating header->number_of_points_by_return["));
        return FALSE;
      }
    }
    stream->seek(header_start_position+179);
...
I64 LASwriterLAS::close(BOOL update_header)
{
  ...
  {
#ifdef _WIN32
    throw std::runtime_error(std::string("WARNING: written "));
#else
    throw std::runtime_error(std::string("WARNING: written "));
#endif
  }
  ...
  if (stream)
  {
    if (update_header && p_count != npoints)
    {
      ...
      {
#ifdef _WIN32
        throw std::runtime_error(std::string("WARNING: stream not seekable. cannot update header from "));
#else
        throw std::runtime_error(std::string("WARNING: stream not seekable. cannot update header from "));
#endif
      }
