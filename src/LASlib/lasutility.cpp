  ...
  if (bin >= 0)
  {
    if (bin >= size_pos)
    {
      ...
      if (size_pos == 0)
      {
        ...
        if (bins_pos == 0)
        {
          throw std::runtime_error(std::string("ERROR: allocating "));
          exit(1);
        }
      ...
      else
      {
        ...
        if (bins_pos == 0)
        {
          throw std::runtime_error(std::string("ERROR: reallocating "));
          exit(1);
        }
...
BOOL LAShistogram::histo(const CHAR* name, F32 step)
{
  ...
  {
    throw std::runtime_error(std::string("ERROR: histogram of '"));
    return FALSE;
  }
  is_active = TRUE;
  return TRUE;
