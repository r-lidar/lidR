...
BOOL LASquadtree::read(ByteStreamIn* stream)
{
  ...
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): wrong LASspatial signature "));
    return FALSE;
  }
  U32 type;
  ...
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): unknown LASspatial type "));
    return 0;
  }
  try { stream->getBytes((U8*)signature, 4); } catch(...)
  {
  ...
  {
//    throw std::runtime_error(std::string("ERROR (LASquadtree): wrong signature "));
//    return FALSE;
...
BOOL LASquadtree::write(ByteStreamOut* stream) const
{
  ...
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing LASspatial type "));
    return FALSE;
  }
  ...
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing levels "));
    return FALSE;
  }
  U32 level_index = 0;
  if (!stream->put32bitsLE((U8*)&level_index))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing level_index "));
    return FALSE;
  }
  U32 implicit_levels = 0;
  if (!stream->put32bitsLE((U8*)&implicit_levels))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing implicit_levels "));
    return FALSE;
  }
  if (!stream->put32bitsLE((U8*)&min_x))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing min_x "));
    return FALSE;
  }
  if (!stream->put32bitsLE((U8*)&max_x))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing max_x "));
    return FALSE;
  }
  if (!stream->put32bitsLE((U8*)&min_y))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing min_y "));
    return FALSE;
  }
  if (!stream->put32bitsLE((U8*)&max_y))
  {
    throw std::runtime_error(std::string("ERROR (LASquadtree): writing max_y "));
    return FALSE;
  }
  return TRUE;
}
...
BOOL LASquadtree::setup(F64 bb_min_x, F64 bb_max_x, F64 bb_min_y, F64 bb_max_y, F32 cell_size)
{
  ...
  {
    throw std::runtime_error(std::string("ERROR: cells_x "));
    return FALSE;
  }
