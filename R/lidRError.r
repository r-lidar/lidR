lidRError = function(code, ..., behaviour = stop)
{
  ...
  msg = switch(code,
               ...
               "TFS4" = list("The expression '", p$expression, "' returned a list in which all elements are not a single value. The field: ", capture.output(cat(p$metric, sep=" and ")), " have respectively a length of: ", capture.output(cat(p$number, sep=" and "))),
