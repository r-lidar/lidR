...
What takes time when processing a LAScatalog is not necessarily the computation itself but the time required to read the files. The read time (i.e the time needed to load the data in R) might be much longer than the actual computation time. This vignette explains why and how to speed-up the computation by a significant factor of 2 to 8 by carefully preparing the catalog.

## Generic considerations on LAScatalog processing

When processing a LAScatalog the covered area is divided into chunks that are then processed sequentially. In the following examples we present the case where chunks are equal to tiles, i.e. when processing each file sequentially. This is the common way to process data but not the only one. Anyway the explaination remains valid even if chunks are not equal to tiles.
...
So let's assume that the total compuation time is 1 unit of time divided into 0.25 unit of actual processing time and 0.75 unit of read time (which is a fairly reasonable ratio). We can divide the read time by 3, and thus have 0.25 units of read time and 0.25 units of computing time, which gives a compuation time is 0.5 instead of 1. We can therefore speed-up the compuation time by a factor of 2 by using the las format instead of laz. Obviouly the gain is less significant for more computationnaly demanding processes.

So to compute faster users can opt for las files instead of laz files. Obviously, there are good reasons to use laz files instead of las files. The strong compression brought by the laz format has a lot of advantages for storing data. It is up to the user to choose a format by considering the trade-offs between space and computation time. This section explains how it works only to help users make a decision that best suits their needs.

## Indexation of the points with lax files

Another way to speed-up the total computation time is to avoid reading the full eight neigbouring tiles to load a buffer. Instead, we can read only sub-part of the neighbouring tiles. The gain comes from the fact that we read on small portion of the neigbouring files to extract the buffer, skipping the majority of the content of the file. Indeed, the buffer usually corresponds to a very very small percentage of the actual content of a file (a few square meters).

This is possible by indexing the las or laz files with lax files. A lax file is a tiny file that can come with a las or laz and that spatially indexes the points to make faster spatial queries. This file type was created by Martin Isenburg in [LAStools](https://rapidlasso.com/). For a better understanding of how it  works one can refer to a talk given by Martin Isenbug about [lasindex](https://rapidlasso.com/2012/12/03/lasindex-spatial-indexing-of-lidar-data/).
...
The gain is really significant and allows an addition 2- to 3-fold saving in terms of read time, which thus speeds-up the computation time. Changing the laz format to the las
format has a cost because it implies storing more data. However, using lax files provides a significant gain for free, so there is no incentive not to create lax files.

## Changing the hardware

We demontrasted how important is the time use to read files to improve the overall computation time. The faster you read the files the faster you perform the compuation because the read time is not negligible. Opting for a fast SSD disk instead of a slow HDD disk may significantly speed-up the compuatation time independently of the power of your processor. Hardware matters!

## Use more cores

This section comes at the end for a purpose. lidR allows you to gain speed using multicore processing. But users will not necessarily have a significant gain with multicore processing. Multicore is not a magic trick. Processing a LAScatalog implies reading a file on disk, a task that is hardly parallelisable. As a consequence, multicore is not necessarily faster than single core. The multicore engine implemented in lidR reads several files at a time to process them in parallel. It uses more memory (four cores means four chunks read at one time) and the overhead of reading four files at a time may be more penalising than the gain. 

In conclusion, multicore processing may or may not speed-up the compuation time and it has a big memory cost. 

## Benchmarks

The following are bechmarks for some functions
...
Here we found an almost 8-fold increase in speed only by changing the file types. With 2 cores we gained 30% more, reaching an actual 8-fold speed-up compared to the basic use of laz files. One can try laz files only + 32 cores on a super computer with a lot of RAM, but the gain will probably not reach an 8-fold speed-up!


#### Area Based Approach

Computation of a single metric on 360 files of 1 x 1 km with 3 pts/m² (~300 km² and 900 millions points) on a laptop with a SSD and a processor intel core i7.
...
Here we found an almost 6-fold speed-up only by changing the file types. With 4 cores we gained an additional 20%, reaching almost a 7-fold speed-up compared to the basic use of laz files. Using 2 cores instead of 4 is likely to be faster. One can try laz files only + 32 cores on a super computer with a lot of RAM, but the gain will probably not reach an 8-fold speed-up!

#### Clip ground inventories

Extraction of 140 plots of 12 m radius from 1 x 1 km files with 3 pts/m² on a laptop with a SSD and a processor intel core i7.
...
Here using 4 cores instead of 1 increases considerably the computation time. Indeed the runtime is very small and the overhead of the parallelization does not compensate for the the potential gain.
