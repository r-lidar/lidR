...
vignette: >
  %\VignetteIndexEntry{LAScatalog formal class}
...
A LAScatalog is formally a SpatialPolygonsDataFrame but at the same time its purpose is not to manipulate spatial data into R. The purpose of a LAScatalog is to represent a set of existing las/laz files. Thus, a LAScatalog cannot be modified because it must be related to the actual content of the files. The following throws an error:
...
When applied to a catalog, the function `lascheck` does not perform a deep inspection of the point cloud, unlike when it is applied to a LAS object. Indeed the point cloud is not actually read.
...
However, processing a LAScatalog usually requires tuning of some processing options to have finer control over the computation. Indeed, if the catalog is huge the output is also likely to be huge and maybe the output cannot fit in R memory. For example, `lasnormalize` throws an error if used 'as is' without tuning the processing options. Using `lasnormalize` as in the following example the expected `output` would be a huge point cloud loaded in memory. The lidR package forbids a call such as:

```{r, error = TRUE, purl = FALSE}
output <- lasnormalize(ctg, tin())
```

Instead one can use the processing option `opt_output_files`. Processing options drive how large files are split into smaller chunks and how the outputs are either returned into R or written into files on disk.
...
Here output is not a point cloud but a LAScatalog pointing to the newly-created files. Users can check how the catalog will be processed by calling `summary`:
...
Load a catalog, process each file sequentially, return a RasterLayer into R.
...
Load a catalog. Process 3 small chunks of 200 x 200 meters at a time. Each chunk is loaded with an extra 20 m buffer. Returns a Shapefile into R.
...
Load a catalog, process small chunks sequentially. For each chunk write a laz file on disk named after the coordinates of the chunk. Returns a lightweight LAScatalog. Notice that the original catalog has been retiled.
...
Load a catalog. Load a shapefile of plot centers. Extract the plots and return a list of extracted plots in R.
...
ctg <- catalog("path/to/las/files/")
shp <- shapefile("plot_center.shp")
opt_output_files(ctg) <- "folder/where/to/store/outputs/plot_{XCENTER}_{YCENTER}_{FORESTTYPE}"
