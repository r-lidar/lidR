...
The following demonstrates how to write your own functions fully applicable on a wide catalog of point clouds and based on the available `lidR` tools. We will create a simple `lasfilternoise` function. This example should not be considered as reference method for filtering noise, but rather as a demonstration to help understand the logic behind the design of lidR, and as a full example of how to create a user-defined function that is fully operationnal. The code design is the one used internally in lidR and it relies on S3 method dispatch. We recomend reading the chapter about [S3 method dispatch](http://adv-r.had.co.nz/S3.html) from the [Advanced R](http://adv-r.had.co.nz/) book.
...
This function is fully functionnal on a point cloud loaded in memory
...
User can access to the catalog processing engine with the function `catalog_apply` that is the engine used internally. It can be applied to any function over an entire catalog. Here we will apply our custom `lasfilternoise` function. To use our function `lasfilternoise` on a LAScatalog we must create a compatible function (see documentation of `catalog_apply`). In the lidR package we usually create an intermediate method. Here `lasfilternoise` for `LAScluster` objects (see also the documentation of `catalog_apply`):
...
1. The processing options were not checked. For example this function should not allow to return the output into R otherwise the whole point cloud will be returned.
