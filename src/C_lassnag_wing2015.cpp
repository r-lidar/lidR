...
// [[Rcpp::export]]
IntegerVector C_Wing2015(S4 las, NumericVector neigh_radii,
                         double low_int_thrsh, double uppr_int_thrsh,
                         int pt_den_req, NumericMatrix BBPRthrsh_mat)
{
   ...
   for (unsigned int i = 0 ; i < n ; i++)
   {
      ...
      ptDen_sph[i] = sphpts.size(); // get the point density in the sphere neighborhood
      ...
      BBPr_cnt = 0; // the count of BBPr points (based on thresholds) in the neighborhood
      ...
      for(unsigned int j = 0; j < smcylpts.size(); j++)
      {
         ...
         {
            ptDen_smcyl[i] +=1; // get the point density in the small cylinder neighborhood for points above focal point
   ...
   for (unsigned int i = 0 ; i < n ; i++)
   {
      ...
      sum_of_elements = 0; // sum the elements in the neighborhood
      double ptZ = Z[i]; // the height of he focal point (lower end of the small cylinder)
      for(unsigned int j = 0; j < smcylpts.size(); j++)
      {
         if(Z[smcylpts[j]->id]>=ptZ)
   ...
   for(unsigned int i = 0; i < n; i++)
   {
      if(ptDen_sph[i]>=pt_den_req && meanBBPr_sph[i]>=BBPRthrsh_mat(0,0) &&
      ptDen_smcyl[i]>=pt_den_req && meanBBPr_smcyl[i]>=BBPRthrsh_mat(1,0) &&
      ptDen_bigcyl[i]>=pt_den_req && meanBBPr_bigcyl[i]>=BBPRthrsh_mat(2,0))
      {
         output[i]=1;  // General snag class
      }
      else if(ptDen_sph[i]>=2 && ptDen_sph[i]<=pt_den_req && meanBBPr_sph[i]>=BBPRthrsh_mat(0,1) &&
      ptDen_smcyl[i]>=2 && ptDen_smcyl[i]<=pt_den_req && meanBBPr_smcyl[i]>=BBPRthrsh_mat(1,1) &&
      ptDen_bigcyl[i]>=2 && ptDen_bigcyl[i]<=pt_den_req && meanBBPr_bigcyl[i]>=BBPRthrsh_mat(2,1))
      {
         output[i]=2; // Small snag class
      }
      else if(ptDen_sph[i]>=pt_den_req && meanBBPr_sph[i]>=BBPRthrsh_mat(0,2) &&
      ptDen_smcyl[i]>=pt_den_req && meanBBPr_smcyl[i]>=BBPRthrsh_mat(1,2) &&
      ptDen_bigcyl[i]>=pt_den_req*7 && meanBBPr_bigcyl[i]>=BBPRthrsh_mat(2,2))
      {
         output[i]=3; // Live crown edge snag class
      }
      else if(ptDen_sph[i]>=pt_den_req && meanBBPr_sph[i]>=BBPRthrsh_mat(0,3) &&
      ptDen_smcyl[i]>=pt_den_req && meanBBPr_smcyl[i]>=BBPRthrsh_mat(1,3) &&
      ptDen_bigcyl[i]>=pt_den_req*15 && meanBBPr_bigcyl[i]>=BBPRthrsh_mat(2,3))
      {
   ...
   return(output); // Return the snagCls are a new column in the las object
}
